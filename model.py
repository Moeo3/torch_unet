from torch.nn import Sequential, ReLU, Conv2d, BatchNorm2d, MaxPool2d, Module, ConvTranspose2d, Sigmoid
...
class UpBlock(Module):
    ...
    def forward(self, inputs, concatenate_part):
        return self.conv(cat([self.up(inputs), concatenate_part], 1))
...
class UNet(Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(self.depth):
            next_inputs = self.up_list[i](next_inputs, concatenate_list[self.depth - i])
